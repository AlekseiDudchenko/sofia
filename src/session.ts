...
/** Сколько ошибок терпит марафон.
 *
 * Три, а не одна: с одной жизнью игра обрывается раньше, чем ребёнок войдёт в
 * ритм, и садиться за неё второй раз не хочется. Три ошибки — это уже понятная
 * цена невнимательности и при этом запас на случайный промах по клавише. */
export const MARATHON_LIVES = 3;

/** Через сколько примеров ошибочный факт возвращается в марафоне.
 *  Ближе, чем в тренировке: очередь здесь бесконечная, и терять факт,
 *  который только что стоил жизни, нельзя. */
export const MARATHON_RETRY_GAP = 3;

/** Факт, который вернётся на экран через `after` примеров. */
export interface Retry {
    fact: Fact;
    after: number;
}

/** Шаг очереди переспросов: счётчики уменьшаются, созревший факт уходит на
 *  экран. Больше одного за раз не отдаём — иначе после двух ошибок подряд
 *  марафон превратился бы в повтор одних и тех же двух примеров. */
export function tickRetries(retries: readonly Retry[]): { due: Fact | null; rest: Retry[] } {
    let due: Fact | null = null;
    const rest: Retry[] = [];

    for (const retry of retries) {
        const after = retry.after - 1;
        if (after <= 0 && !due) due = retry.fact;
        else rest.push({ fact: retry.fact, after });
    }
